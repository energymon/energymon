...
#ifdef ENERGYMON_DEFAULT_CRAY_PM_CPU_ENERGY
#include "energymon-default.h"
int energymon_get_default(energymon* em) {
  return energymon_get_cray_pm_cpu_energy(em);
}
#endif

int energymon_init_cray_pm_cpu_energy(energymon* em) {
  return energymon_cray_pm_common_init(em, ENERGYMON_CRAY_PM_COUNTER_CPU_ENERGY);
}
...
char* energymon_get_source_cray_pm_cpu_energy(char* buffer, size_t n) {
  return energymon_strencpy(buffer, "Cray Power Monitoring - cpu_energy", n);
}
...
int energymon_get_cray_pm_cpu_energy(energymon* em) {
  ...
  em->finit = &energymon_init_cray_pm_cpu_energy;
  em->fread = &energymon_read_total_cray_pm_cpu_energy;
  em->ffinish = &energymon_finish_cray_pm_cpu_energy;
  em->fsource = &energymon_get_source_cray_pm_cpu_energy;
  em->finterval = &energymon_get_interval_cray_pm_cpu_energy;
  em->fprecision = &energymon_get_precision_cray_pm_cpu_energy;
  em->fexclusive = &energymon_is_exclusive_cray_pm_cpu_energy;
  em->state = NULL;
  return 0;
}
